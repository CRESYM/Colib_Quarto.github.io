...
categories: [Opensource, RMS, phasor, TMRL2, voltage source, converter, wind, pv, hdvc, dynawo, STEPSS ] 
date: 17/05/2024 
...
![Grid following converter](./img/grid_following_vsc_model.png){fig-align="center"}
...
![vsc control reference frames](./img/vsc_control_reference_frame.png){fig-align="center"}

::: {.figure-caption}
VSC control reference frames
:::
...
$$
\begin{align}
v_d &= v_x \cdot \cos(\tilde{\delta}_g) + v_y \cdot \sin(\tilde{\delta}_g) \\
i_d &= r \cdot i_x \cdot \cos(\tilde{\delta}_g) + r \cdot i_y \cdot \sin(\tilde{\delta}_g)
\end{align}
$$
...
$$
\begin{align}
L_{pr} \frac{di_d}{dt} &= \omega_N \cdot ( v_{md} - \frac{v_d}{r} - R_{pr} i_d + \tilde{\omega}_g L_{pr} i_q)\\
L_{pr} \frac{di_q}{dt} &= \omega_N \cdot ( v_{mq} - \frac{v_q}{r} - R_{pr} i_q + \tilde{\omega}_g L_{pr} i_d)
\end{align}
$$
...
![PLL](./img/PLL.png){fig-align="center"}
...
![dq current control](./img/dq_current_control.png){fig-align="center"}
...
![active power control](./img/active_power_control.png){fig-align="center"}
...
- $I_{max}$ is the maximum allowed current (in pu). It could be slightly larger than 1
...
![reactive power control](./img/reactive_power_control.png){fig-align="center"}
...
- $𝑉_{𝑐𝑜𝑚𝑝}$ is the compensated voltage :
  $$V_{comp}= | \frac{\bar{V}}{r} + (R_c + j X_c) r \bar{I} |$$
...
![id current limitations](./img/iqmax_limitations.png){fig-align="center"}
