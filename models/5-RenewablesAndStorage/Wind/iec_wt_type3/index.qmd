...
categories: ["regulations", "WT", "wind", "controller", "Opensource", "IEC", "RMS", "modelica", "dynawo", "#62"]
date: 20/12/2024
...
developed with the following specifications in mind [@IEC61400-27-1_2020]:
...
Other model considerations according to [@IEC61400-27-1_2020]:
...
The IEC 61400-27-1 [@IEC61400-27-1_2020] defines one of the most widely used series of
...
drawings are taken from [@IEC61400-27-1_2020; @Honrubia-Escribano2018_review; @Fortmann2015; @Fortmann2014_new_generic; @Lorenzo-Bonache2017_energies; @Lorenzo-Bonache2019_iet].
...
Specifically, [@IEC61400-27-1_2020] defines the following validity domain:
...
- frequency disturbances, variations ±6%
...
- According to [@Honrubia-Escribano2018_review], the Type 3 WT model can operate with variable
speed with slip values from -0.3 to +0.4.

Vice versa, [@IEC61400-27-1_2020] states that the models are not intended for the
following:
...
![Wind turbine type 3 model, based on @IEC61400-27-1_2020](drawings/WT_system.drawio.svg){#fig-wtSystem}

![Wind turbine generator control sub-structure for Type 3A and 3B WT based on @IEC61400-27-1_2020](drawings/WT_generator_control_substructure_type3.drawio.svg){#fig-wtControlSubstructure}


### P control module {#sec-pControlModule}

For detailed assessment of the parameters' impact on model behaviour,
see @Lorenzo-Bonache2017_energies.


![Wind turbine P control module (Type 3), based on @IEC61400-27-1_2020](drawings/WT_P_control_type3.drawio.svg){#fig-wtPControl}
...
should rotate when it is injecting a certain active power [@Lorenzo-Bonache2019_iet]. A
...
[@DIgSILENT2024_powerfactory].

The power-speed-characteristic in
@fig-lookup-table-omega-pref
has four main operating zones according to @Lorenzo-Bonache2017_energies:
...
solid lines. [@Lorenzo-Bonache2017_energies]
:::
...
![Lookup table for reference rotation speed as a function of WT active power, according to @Lorenzo-Bonache2017_energies and with data from @DIgSILENT2024_powerfactory (see @tbl-wtLookupTable)](drawings/fig-lookup-table-omega-pref-output-1.png){#fig-lookup-table-omega-pref}

The power-speed-characteristic's speed reference output is then filtered
($T_\mathrm{\omega ref}$) to avoid sudden changes.

:::{.callout-note}
The time constant $T_\mathrm{\omega ref}$ can be set to a very high value to enforce a fixed reference rotational speed during the simulation, which can be a desired operational mode. [@Lorenzo-Bonache2017_energies]
:::
...
$\omega_\mathrm{gen}$ [@Lorenzo-Bonache2017_energies].
...
[@Lorenzo-Bonache2019_iet].

Another input to the torque PI subsystem (see @fig-torquePi) is the
maximum electromagnetic torque $\tau_\mathrm{emax}$. It is determined by
calculating a torque value from [@Lorenzo-Bonache2017_energies]:
...
$\omega_\mathrm{gen}$ to obtain a power reference [@Lorenzo-Bonache2019_iet].
...
(either side of the gearbox) [@Lorenzo-Bonache2017_energies]. It is modeled through a second-order
...
oscillating electrical power that has an efficient damping effect [@Lorenzo-Bonache2017_energies]
(for parameters see @tbl-parameters).


**Torque PI block**


![Wind turbine torque PI block (Type 3)](drawings/WT_torque_pi.drawio.svg){#fig-torquePi}

::: {.figcaption}
Figure 5: Wind turbine torque PI block (Type 3), based on @IEC61400-27-1_2020
:::

In @fig-torquePi the Torque PI Block can be seen. It acts as a PI-controller and is a subsystem of the active power control in @fig-wtPControl @Lorenzo-Bonache2019_iet.
...
the @sec-pControlModule section above.
...
following two signals [@Lorenzo-Bonache2019_iet]:
...
$F_\mathrm{o}$ steps back to 0. @Lorenzo-Bonache2019_iet
...
- $F_\mathrm{reset}$, while true,
  ...
  - changes the upper ramp rate limit of $\tau_\mathrm{reset}$ to
    ...
    $\tau_\mathrm{reset}$ from inceasing during fault) @Lorenzo-Bonache2019_iet
...
the current limitation system begins to act. See @Lorenzo-Bonache2019_iet for further
details.
...
with a maximum rate dictated by $\mathrm{d}\tau_\mathrm{max}$ @Lorenzo-Bonache2019_iet.

#### Variable limits integrator with set/reset and freeze

The integrators used in the Torque PI model in @fig-torquePi need variable
...
Typical values were gathered from the *DIgSILENT PowerFactory* @DIgSILENT2024_powerfactory
...
| $K_{\mathrm{DTD}}$                  | float | $pu$   | $S_{\mathrm{base}} / \Omega_{\mathrm{base}}$              | Active drive train damping: gain                                                                                                                | $1.5$ (or $0$ if $M_\mathrm{\omega Tqpi}=\mathrm{false}$) [@Lorenzo-Bonache2017_energies]      |
...
| $M_\mathrm{\omega Tqpi}$            | bool  | $-$   | $-$                                                          | Mode for source of rotational speed for torque PI controller error calculation $(false: \omega_{\mathrm{gen}} - true: \omega_{\mathrm{WTR}})$ | $true$ (or $false$ if $K_\mathrm{DTD}=0$) [@Lorenzo-Bonache2017_energies]                      |
...
| $\omega_{\mathrm{DTD}}$             | float | $pu$   | $\Omega_{\mathrm{base}}$                                  | Active drive train damping: frequency, derived from two-mass model parameters, see @eq-omegaDtd    | $11.3$                                                             |
...
| $\omega(p)$                         | float | $pu$   | $\Omega_{\mathrm{base}}(S_{\mathrm{base}})$               | Lookup table for power as a function of speed                                                                                                   | see @tbl-wtLookupTable |
...
: Parameters of WT P control module, based on [@IEC61400-27-1_2020], [@Lorenzo-Bonache2017_energies] and [@DIgSILENT2024_powerfactory] {#tbl-parameters}

:::{.callout-note}
According to [@IEC61400-27-1_2020], using rotor speed instead of generator speed (by
...
: Typical values for lookup table $\omega(p)$, based on [@DIgSILENT2024_powerfactory] implementation {#tbl-wtLookupTable}
...
: Inputs, based on [@IEC61400-27-1_2020] and [@Lorenzo-Bonache2017_energies] {#tbl-inputsPControl}
...
: Outputs, based on [@IEC61400-27-1_2020] {#tbl-outputsPControl}
...
$$x_\mathrm{pord\,0} = P_\mathrm{ord\,0}$$ {#eq-initPOrd}

$$x_\mathrm{DTD1\,0} = x_\mathrm{DTD2\,0} = 0 \text{ (make sure to obtain steady zero output)}$$ {#eq-initDtd}

$$x_\mathrm{\omega ref\,0} = \mathbf{\omega}(P_\mathrm{0})$$ {#eq-initOmegaRef}

$$x_\mathrm{\omega filtp3\,0} = \omega_0$$ {#eq-initOmegaFiltP3}

$$x_\mathrm{dprefmax\,0} = P_\mathrm{WTRef\,0}$$ {#eq-initDPRefMax}
...
$$x_\mathrm{d\tau max\,0} = \tau_\mathrm{emax\,0}$$ {#eq-initDTauMax}
...
![Wind turbine Q control module, based on @IEC61400-27-1_2020](drawings/WT_Q_control.drawio.svg){#fig-QControlModule}
...
The following descriptions have been based on @Franke2024 and @IEC61400-27-1_2020.
...
Special attention must be paid to the reactive current signs in @IEC61400-27-1_2020,
as they seem to be inconsistent @Franke2024. In the fast injection path,
...
: Q control normal path control modes, based on @IEC61400-27-1_2020 {#tbl-qControlModesNormal}
...
: Q control reactive current injection path control modes, based on @IEC61400-27-1_2020 {#tbl-qControlModesFast}
...
: Parameters of WT Q control, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersQControl}
...
: Inputs, based on @IEC61400-27-1_2020 and @Lorenzo-Bonache2017_energies {#tbl-inputsQControl}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsQControl}
...
![WT Q limitation module, based on @IEC61400-27-1_2020](drawings/WT_Qlim.drawio.svg){#fig-WTQLim}
...
: Parameters of Q limitation module, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersWTQlim}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsWTQlim}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsWTQlim}
...
Outside FRT, P-Priority is always active regardless of $M_\mathrm{qpri}$.
...
The following [code @lst-currentLimCode] shows a python inspired pseudo-code implementation of the the current limitation as shown in @Franke2024.

![Wind turbine current limitation, based on @IEC61400-27-1_2020](drawings/WT_CurrentLim.drawio.svg){#fig-WTCurrentLim}

```{#lst-currentLimCode .default lst-cap="Python-inspired pseudo code implementation of the current limitation block, based on @Franke2024"}
...
: Parameters of current limitation module, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersCurrentLim}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsCurrentLim}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsCurrentLim}
...
![Wind turbine two mass mechanical module, based on @IEC61400-27-1_2020](drawings/WT_mechanical.drawio.svg){#fig-mechanicalModule}
...
: Parameters of mechanical module, based on @IEC61400-27-1_2020 and @Tsourakis2009 {#tbl-parametersMechanical}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsMechanical}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsMechanical}
...
![Wind turbine two-dimensional aerodynamic module, based on @IEC61400-27-1_2020](drawings/WT_aerodynamic.drawio.svg){#fig-wtAerodynamic}
...
resulting in the wind turbine aerodynamic power $p_\mathrm{aero}$ @Fortmann2014_new_generic.

**Parameters**

Typical values were gathered from the *DIgSILENT PowerFactory* @DIgSILENT2024_powerfactory implementation of the model.
...
: Parameters, based on @IEC61400-27-1_2020 {#tbl-parametersAero}

::: {.callout-note}
According to @IEC61400-27-1_2020, for $0 < p_\mathrm{avail} < 1$ the pitch angle
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsAero}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsAero}

### Pitch angle control module

![Wind turbine pitch angle module (Type 3), based on @IEC61400-27-1_2020](drawings/WT_pitch.drawio.svg){#fig-wtPitchAngleControl}

The pitch angle control module is shown in
@fig-wtPitchAngleControl. It
...
and one with generator power ($K_\mathrm{Ic}$, $K_\mathrm{Pc}$) @Honrubia-Escribano2018_review.
...
fast the aerodynamic power is reduced @Honrubia-Escribano2018_review.
...
introduced into the rotor speed error @Honrubia-Escribano2018_review.
...
| $\mathrm{d}\theta_{\mathrm{cmin}}$      | float | deg/s | \-                                                     | Pitch minimum positive ramp rate of power PI controller | -3             |
...
: Parameters, based on @IEC61400-27-1_2020 {#tbl-parametersPitch}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsPitch}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsPitch}
...
Two different models (3A and 3B) have been definded in the IEC @IEC61400-27-1_2020,
...
the DC circuit of the converter @Honrubia-Escribano2018_review. Rotor and converter protection
...
@Fortmann2014_new_generic. This leads to a delay on the delivery of voltage control
functionality as well as active and reactive power transients after
fault inception and fault clearance @Honrubia-Escribano2018_review.

##### Type 3A


![Wind turbine type 3A generator system model, based on @IEC61400-27-1_2020](drawings/WT_GS_3A.drawio.svg){#fig-generatorType3a}
...
@Honrubia-Escribano2018_review @Lorenzo-Bonache2019_iet.

The air-gap power $p_\mathrm{ag}$ is an additional output of the model,
which is used as an input for the mechanical model in @fig-mechanicalModule.

In this model a DC chopper is assumed. The model is not applicable for
older DFIG systems that require a crowbar during grid faults @Fortmann2015,
p. 111).

##### Type 3B

![Wind turbine type 3B generator system model, based on @IEC61400-27-1_2020](drawings/WT_GS_3B.drawio.svg){#fig-generatorType3b}

The Type 3B Generator System models a DFIG with crowbar circuit.

In the model, as shown in
@fig-generatorType3b, the
...
: Parameters, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersGenSys3a}
...
: Parameters, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersGenSys3b}
...
: Inputs, based on @IEC61400-27-1_2020 and @Lorenzo-Bonache2017_energies {#tbl-inputsGensys}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsGensys}
...
![Grid measurement module](drawings/IEC_WT_meas.drawio.svg)
...
: Parameters, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersMeasurement}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsMeasurement}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsMeasurement}
...
conditions. As desribed in @IEC61400-27-1_2020, this time-based grid protection has
...
: Parameters, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersProtection}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsProtection}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsProtection}

## Wind power plant model


![Wind plant control and communication, based on @IEC61400-27-1_2020](drawings/WP_control_and_communication.drawio.svg){#fig-wp}

The Wind Plant (WP) model includes two controllers: active power / frequency controller (@fig-wpActivePower) and and reactive power / voltage control (@fig-wpReactivePower).
...
According to @IEC61400-27-1_2020 it is normally enough to use a single aggregated
...
![WPP P control module, based on @IEC61400-27-1_2020](drawings/WP_P.drawio.svg){#fig-wpActivePower}


The active power controller controls P as a function of frequency or
external reference changes @IEC61400-27-1_2020.
...
: Parameters of WP P control, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersWPP}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsWPP}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsWPP}
...
![WPP Q control module, based on @IEC61400-27-1_2020](drawings/WP_Q.drawio.svg){#fig-wpReactivePower}

The reactive power controller can control power factor, reactive power,
or voltage @IEC61400-27-1_2020. Reactive power capability limitations are taken into
...
: Parameters of WP Q control, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersWPQ}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsWPQ}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsWPQ}
...
![Linear communication delay module, example with 3 delayed variables, based on @IEC61400-27-1_2020](drawings/WP_communication.drawio.svg){#fig-wpCommunication}
...
: Parameters of linear communication module, based on @IEC61400-27-1_2020 and @DIgSILENT2024_powerfactory {#tbl-parametersLinearCommunictation}
...
: Inputs, based on @IEC61400-27-1_2020 {#tbl-inputsLinearCommunication}
...
: Outputs, based on @IEC61400-27-1_2020 {#tbl-outputsLinearCommunication}


## Base values

The following base values are defined according to @IEC61400-27-1_2020:
