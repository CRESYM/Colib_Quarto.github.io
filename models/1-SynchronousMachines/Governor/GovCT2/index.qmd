...
layout: page
title: GovCT2 / GGOV2
categories: ["regulations", "governor", "controller", "synchronous machine", "Opensource", "IEC", "CIMDynamic", "RMS", "modelica", "dynawo", "RMS", "#53"]
date: 23/04/2024
...
manufacturer. Both are based on *Rowen's model* from 1983 [@Machowski2020].

When comparing to older standards: GovCT2 is identical to GGOV2 and
GovCT1 is identical to GGOV1.

GovCT2 is part of the CIM/CGMES standard, see [@Alliander2024] and [@IEC61970-302-2023]. CIM is
...
The following information has been gathered from [@Alliander2024], [@Neplan2015] and [@IEC61970-302-2023].
...
![Model schema, based on [@IEC61970-302-2023]](drawings/GovCT2.drawio.svg){#fig-modelSchema}
...
deactivated by setting parameters to zero (examples in [@Machowski2020]).

**Supervisory load controller**

In [@IEC61970-302-2023] the $P_{MWSet}$ path is described as an optional
...
slow reaction compared to the speed governor. [@IEC61970-302-2023]

A value $K_{I,MW}$ = to 0.01 corresponds to a time constant of
100 s; 0.001 corresponds to 1000 s (relatively slow acting) [@IEC61970-302-2023].
...
load limit, set the parameter $P_{ldref}$ to a high value [@IEC61970-302-2023].
...
controller [@IEC61970-302-2023]. Additionally there is a dynamic frequency dependent
limit reduction, see [Frequency dependent (valve) limit](#frequency-dependent-valve-limit) section.

For a gas turbine, in the presence of a minimum firing limit,
$V_{min}$ normally is set greater than zero and less than
$W_{fnl}$ [@IEC61970-302-2023].

The value of the fuel flow at maximum power shall be $\leq 1$, depending
on the value of $K_{turb}$ [@IEC61970-302-2023]. It translates the fuel
consumption (or water flow) to mechanical power output [@Machowski2020].
...
transport delay is of particular concern [@IEC61970-302-2023].
...
[@IEC61970-302-2023]. If True, fuel flow is proportional to speed. This is applicable
...
If $D_m=0$, the speed sensitivity paths are not active. [@IEC61970-302-2023]
...
aeroderivative turbines [@IEC61970-302-2023]).
...
temperature limits) [@IEC61970-302-2023]. The authors suspect that this could represent
fan cooling.
...
$V_{max}$ [@IEC61970-302-2023].

![Frequency dependent valve limit as described in [@IEC61970-302-2023]](drawings/GovCT2.frequencylimit.drawio.svg){#fig-frequencyDependentLimit}
...
| $\Delta\omega_{db}$  | float | $pu$     | $db$           | Frequency error deadband. Recommended to be =0 in most applications [@IEC61970-302-2023]                                    | $0$             |
