...
stability and control* [@Kundur] (Fig. E12.8).

## Table of references

For additional references see the bibliography and the following sources:
...
![Kundur's two-area test system](drawings/Kundur_two_area.drawio.svg){#fig-two-area-system}

In @fig-two-area-system the
test system is shown. As [@Kundur] describes it, it consists of two similar
...
- AVR DC1A (similar to DC1C, [see colib page
  here](../../models/1-SynchronousMachines/ExciterAndVoltageRegulator/DC1C/index.qmd))
  - $K_\mathrm{a}=$ $20$ $pu$
  - $K_\mathrm{a}=$ $200$ $pu$
- AVR ST1A (similar to ST1C, [see colib page
  here](../../models/1-SynchronousMachines/ExciterAndVoltageRegulator/ST1C/index.qmd))
...
@eq-zipload-p and
@eq-zipload-q define ZIP
load model. Depending on the coefficients in
@tbl-zip-load-parameters it
...
In [@Kundur] it is stated that the active power should be modelled as
...
: Synchronous machine parameters  {#tbl-sm-parameters}
...
: AVR DC1A parameters {#tbl-dc1a-parameters}
...
: AVR ST1A parameters {#tbl-st1a-parameters}
...
: PSS parameters {#tbl-pss-parameters}
...
: Step-up transformer parameters  {#tbl-trafo-parameters}
...
: Grid parameters {#tbl-grid-parameters}
...
: Load parameters {#tbl-load-parameters}


## Scenarios

The following sceniarios are taken from [@Kundur] and related references, which are also
...
: Loadflow voltages of Dynawo implementation {#tbl-loadflowU}
...
: Loadflow generator powers of Dynawo implementation {#tbl-loadflowS}


- The active power exchange from Area 1 to Area 2 (see
  @fig-two-area-system) is
...
@fig-example-a-eigenvalues
shows the result of an eigenvalue analysis of this system, compared to
the reference values from [@Kundur]. The remaining differences probalby
...
![Eigenvalues of the system with constant excitation voltage](images/20240814123344.png){#fig-example-a-eigenvalues}

### Example b

:::{.callout-note}
To obtain a steady state initial condition, the exact values for the generators active and reactive power and voltage angles from load flow [Section 6.1](#load-flow) need to be used as initial values in the dynamic simulation.
:::

:::{.callout-note}
In the Dyna$\omega$o implementation, if the voltage transducer time constant $T_\mathrm{R}$ = $0.01$ $s$, then the voltage measurement first order lag and the field flux linkage $\lambda_\mathrm{f}$ oscillate against each other with high amplitude. This is not represented in the reference results. To avoid this behaviour, set $T_\mathrm{R} = 10^{-6}$ $s$. This has been done for the Dyna$\omega$o implementation.
:::
...
In related references this case has not been taken into account for comparative
...
@tbl-sm-parameters because its
synchronous machine model uses a different saturation model. Instead of
the model from Kundur [@Kundur], the Shackshaft model is used. To get
...
![Unstable eigenvalue of the system with exciter ST1A](images/20240814131709.png){#fig-example-b-2-eigenvalues}

As in related references, a steps to the synchronous machines' voltege references
have been applied according to
@tbl-example-b-steps. Those
were selected to excite the inter-area oscillation mode and the other
electromechanical modes in the system.
...
: Voltage reference steps {#tbl-example-b-steps}


The step response of the generators can be seen in
@fig-example-b-2-stepresponse.


![Unstable step response of the system with ST1A without transient gain reduction](images/20240814133617.png){#fig-example-b-2-stepresponse}

**Part 3: Thyristor exciter (ST1A) with transient gain reduction (TGR)**

Simlilarly to
[Section 6.3.2](#example-b), the ST1A
...
[@Kundur] (Figure 6-8) is activated and parameterized according to
@tbl-pss-parameters.
