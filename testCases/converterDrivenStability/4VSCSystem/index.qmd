...
categories: [opensource, small, TCRL2, converter driven stability, HVDC, cable, WTG, EMTP, STEPSS, dynawo] 
date: 02/01/2024  
...
The one-line diagram is shown in the following figure:

![Four VSC system](oneline.png){#fig-oneline}
...
| | $R$ ($\Omega/km$) | $X$ ($\Omega/km$) | $\omega \cdot \frac{C}{2}$ ($\mu S/km$)| $S_{nom}$ ($MVA$) |
| -- | --------------- | ------------------ | ------------------------------------ | ----------------|
| 400-kV* | $0.016$ | $0.320$ | $1.5$ | $1300$ |
| 225-kV** | $0.0084$ | $0.017$ | $180.0$ | $2400$ |

: line and cable data {#tbl-line-cable}

\* pertain to each circuit
** pertain to 6 cables in parallel

| Transformer | Nominal Voltage ($V$) | $R$ ($pu$) | $X$ ($pu$) | $n$ | $S_{nom}$ ($MVA$) |
| ----------- | --------------- | --------- | ---------- | ------------- | -----------|
...
: Transformer data {#tbl-transformer}

| Converter | $S_{nom}$ ($MVA$) | $P_{nom}$ ($MVA$) |
| ----------- | ---------- | ------ |
| WP1 | $2400$ | $2300$ |
| WP2 | $2400$ | $2300$ |
| HVDC1 | $1200$ | $1150$ |
...
Two operating points were considered in the simulations, as detailed in @tbl-operating-points.
...
losses) is taken by the load at bus $C$, which results in no power flowing into the Thévenin equivalent. Hence,
...
![Operating point #2](OP2.jpg){#fig-op2}
...
The voltage evolutions are given in @fig-voltages. During the fault, the voltage at bus A drops to zero, due to
...
![Operating point #1: voltages](volt.jpg){#fig-voltages}


The evolution of the VSC active powers is shown in @fig-active-powers (left part). During the fault, the active power
...
setpoint values of the $i_d$ components. These setpoints are the input the fact current control loops.

![Operating point #1: active powers and $i_d$ current setpoints](P.jpg){#fig-active-powers}

The evolution of the reactive powers is shown in the left part of @fig-reactive-powers while the setpoint values of the $i_q$ components are shown in the right part. During the fault, all VSCs have their reactive power increased but their $i_q$ components evolve differently:

- WP2, HVDC1 and HVDC2 control their voltages (on the converter side of the connection transformer).
The severe voltage drop caused by the fault is counteracted by a quick decrease of $i_q$, which corresponds to maximum injection of reactive power. $i_q$ hits the -1 limit, which corresponds to the whole current being taken by the $i_q$ component ($i_d = 0$), in accordance with the reactive power priority;
...
![Scenario #1: reactive powers and $i_q$ current setpoints](Q.jpg){#fig-reactive-powers}
...
The limit value of the short-circuit power obtained with respectively EMT and phasor simulations have been compared. The marginal cases are shown in @fig-emt-marginal for the EMT simulation and @fig-phasor-marginal for the phasor-mode simulation. It can be seen that in EMT simulations the system becomes unstable for a short-circuit power around 16 GVA, while in phasor-mode simulations it becomes unacceptable at 5.5 GVA. There is thus a very significant difference. This scenario is very appropriate for future analysis in the BiGER (CRESYM) project.

![Scenario #2: EMT simulations in marginal cases](stab_limit_EMT.jpg){#fig-emt-marginal}

![Scenario #2: Phasor-mode simulation in marginal cases](stab_limit_phasor.jpg){#fig-phasor-marginal}
